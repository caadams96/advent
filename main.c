...
int main(){
    register int j,k = 0;
    register char*p;
    register instruction*q = NULL,*qq;
    register object t;
     ...
     * large objects, which appear in more than one location, are listed more than once using ‘_’.
     *
     * */
    build_object_words();
     ...
     * If an action does not meet special conditions,
     * we will issue a default message.
     * */
    build_action_words();
    ...
    * */
     ...
     * in the group containing object t. Objects that belong to groups are immovable; they always stay in the same
     * location. Other objects have base [t] = NOTHING and they are free to leave one list and join another. For
     * example, it turns out that the KEYS are movable, but the SNAKE is always in the Hall of the Mountain King;
     * we set base [KEYS] = NOTHING and base [SNAKE] = SNAKE. Several groups, such as the GRATE and GRATE_,
     * consist of two objects. This program supports operations on groups of more than two objects, but no such
     * objects actually occur.
     * Each movable or base object t has a current property prop[t], which is initially −1 for treasures, otherwise
     * initially 0. We change prop[t] to 0 when you first see treasure t; and property values often change further as
     * the game progresses. For example, the PLANT can grow. When you see an object, we usually print a message
     * that corresponds to its current property value. That message is the string note [prop[t] + offset[t]].
     * (Exception: When you first see the RUG or the CHAIN, its property value is set to 1, not 0. The reason
     * for this hack is that you get maximum score only if the property values of all treasures are zero when you
     * finish.)
     * Each object is in at most one list, place [t]. If you are carrying object t, the value of place [t] is inhand ,
     * which is negative. The special location limbo has value 0; we don’t maintain a list first[limbo] for objects
     * that have place [t] = limbo. Thus object t is in a list if and only if place [t] > 0. The global variable holding
     * counts how many objects you are carrying.
